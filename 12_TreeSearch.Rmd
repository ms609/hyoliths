...
for (i in seq_along(iw.trees)) {
  iTrees <- iw.trees[[i]]
  if (length(iTrees) > 0) allTrees <- c(allTrees, iTrees)
}

powersOf2 <- 2^(seq_along(allTrees[[1]]$tip.label)- 1)
...
ColPlot(RootTree(consensus(lapply(iw.trees, consensus)), rootingTips))
...
PlotPanel <- function (i) {
  if (i > length(iw.trees)) {
    ...
    ColPlot(RootTree(consensus(iw.trees[[i]]), rootingTips))
