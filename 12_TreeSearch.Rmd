...
iw.trees <- lapply(kValues, function (k) {
  ...
  if (length(iw.best) == 0) {
    ...
    if (class(loadedTrees) == 'multiPhylo') unique(loadedTrees) else loadedTrees
  }
})

allTrees <- ew.trees
for (i in seq_along(iw.trees)) {
  allTrees <- c(allTrees, iw.trees[[i]])
}
...
omit <- c("Micrina", 'Mickwitzia_muralensis',
       'Heliomedusa_orienta')
majCon <- RootTree(ConsensusWithout(allTrees, omit, p=0.5), rootingTips)
conTip <- Ntip(majCon)
splits <- Descendants(majCon, conTip + seq_len(Nnode(majCon)), type='tips')
splits <- lapply(splits, function (x) sort(majCon$tip.label[x]))
internalNodes <- conTip + seq_len(conTip - 1)
...
supporters <- vapply(splits, function (tips) sum(vapply(prunedTrees, function (tree) {
  ...
  prunedTips <- tree$tip.label
  treeSplits <- vapply(treeSplits[splitLengths == length(tips)],
                       function (x) sort(prunedTips[x]), character(length(tips)))
