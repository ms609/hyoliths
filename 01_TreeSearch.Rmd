...
```{r Implied weights results in TreeSearch, fig.cap="Strict consensus of equal weights results", fig.width=7.1, fig.height=11}
# Plot consensus results
par(mfrow=c(4, 2), mar=rep(0.2, 4))

omit=c(lon, 'Clupeafumosus_socialis', 'Heliomedusa_orienta', 'Haplophrentis_carinatus')
ColPlot(ConsensusWithout(lapply(iw.trees, consensus), omit))
ColMissing(omit)
text(-0.5, 0.2, pos=4, "Consensus of all k values", cex=0.8)
