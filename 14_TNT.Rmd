...
(ref:tnt-iw-results) Strict consensus of all trees recovered by TNT using Fitch parsimony with implied weighting.  The consensus of all implied weights runs is not very well resolved. 
...
```{R tnt-iw-16-24, fig.width=7.1, fig.height=4, echo=FALSE, fig.cap='(ref:tnt-iw-results)'}
par(mfrow=c(1, 2))
plotPanel(6)
plotPanel(7)
```
...
ColPlot(ConsensusWithout(ReadTntTree(tntEw, relativePath='.'), omit))
ColMissing(omit)
```
