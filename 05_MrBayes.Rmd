...
Four MrBayes runs were executed, each sampling eight chains for `Mgen`&nbsp;000&nbsp;000 generations, with samples taken every 500 generations.  The first 10% of samples were discarded as burn-in.
...
pstat <- read.table('MrBayes/hyo.nex.pstat', skip=0, header=TRUE)[, -c(4:6)]
pstat[, 2:5] <- signif(pstat[, 2:5], 3)
pstat[, 6] <- signif(pstat[, 6], 5)
knitr::kable(pstat, caption="MrBayes parameter estimates (.pstat file)")
```
