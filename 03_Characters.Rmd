...
LookUpReference <- function (keys, original, prefix='', suffix='') {
  ...
  keys <- gsub("^(.+?)\\b.*$", "\\1", keys, perl=TRUE)
  if (length(keys) == 0) {
    return (character(0))
  } else if (length(keys) == 1) {
    matches <- grepl(keys, refKeys, fixed=TRUE)
    if (sum(matches) == 1) {
      paste0(prefix, refKeys[matches], suffix)
    } else {
...
ConvertReferences <- function (string) {
  # TODO: Fix "Williams et al. (1997, 2000)"
  # TODO: Fix "(Williams et al. 1997; 2000)"
  # TODO: Fix "Zhang et al. (2007A, 2011T)"
  # TODO: Fix "(Williams et al. 1997 p. 418)"
  # TODO: Fix "Williams et al. (1998T, p. 250)."
  # A bridge too far? "(Williams et al. 1998T, p. 250; 2000, p. 102)."
  ...
  ReplPar <- function (keyedText, oldKey, original) {
    StrReplace(keyedText, oldKey,
                LookUpReference(oldKey, original, 
                  ...
                  suffix=paste0(gsub(parentheses, "\\5", original, perl=TRUE), ']')
                ), vectorize_all=FALSE)
  ...
  ReplaceRefs <- function (pattern, strings, Replace) {
    ...
    keyed <- gsub(pattern, "~~~\\1\\3\\5~~~", strings, perl=TRUE)
    keys <- lapply(matchText, function (x) unique(gsub(pattern, "~~~\\1\\3\\5~~~",
                                                       x, perl=TRUE)))
