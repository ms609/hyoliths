...
PrintCharacter <- function () {
  cat(Header(3L, paste0("[", i, "] ", paste0(
    this_char_parts[if(char_nesting[i] < 4) char_nesting[i] else
      3:char_nesting[i]], collapse=': '), " {-}")))
}
...
for (i in seq_len(n_char)) {
  ...
  if ( # Reasons to print a heading
    ...
    ( this_nesting > 1L &&
      prev_char_parts[2L] != this_char_parts[2L] && 
      this_char_parts[2] == next_char_parts[2]
    )
  ) {
    if ( # Reasons to print a Solo character
      ...
      (this_nesting == 2L && next_nesting > 1L &&
            next_char_parts[2] == this_char_parts[2]) ||
