...
LookUpReference <- function (keys, fail, Format) {
  ...
  ret[uniques] <- apply(matches[, uniques, drop=FALSE], 2, function (x) Format(refKeys[x]))
...
ConvertReferences <- function (string) {
  ...
  ReplNoPar <- function (keyedText, oldKey) {
    str_replace(keyedText, oldKey, 
                LookUpReference(oldKey, matchText[[i]], function(x) paste0('@', x)),
                vectorize_all=FALSE)
  }
  
  parentheses <- "([A-Z][A-z]+)( et al\\.)? \\((\\d\\d\\d\\d)\\)"
  ReplPar <- function (keyedText, oldKey) {
    str_replace(keyedText, oldKey, 
                LookUpReference(oldKey, matchText[[i]], function (x)
                  paste0(gsub(parentheses, "\\1\\2", matchText[[i]]), ' [-@', x, ']')),
                vectorize_all=FALSE)
  }
