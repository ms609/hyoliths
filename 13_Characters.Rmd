...
if (knitr::is_html_output()) {
  ...
  trees <- c(ew.trees, iw.trees[[1]], iw.trees[[2]],
...
if (knitr::is_html_output()) {
  ...
  nTrees <- sum(nTree)
  treeIndex <- cumsum(nTree)
  treeOffset <- treeIndex - nTree
  TreeLink <- function (iNo, offset) {
    globalNo <- offset + iNo
    paste0('<span class="showTree" id="showTree', globalNo, '" onclick="showTree(', 
           globalNo, ', ', n_char, ');">[', iNo, ']</span>')
  }
  rows <- vapply(seq_along(nTree), function (i) {
    paste(TreeLink(1:nTree[i], treeOffset[i]), collapse=' ')
  }, '')
...
for (i in seq_len(n_char)) {
  ...
  lapply(seq_along(trees), function (treeNo) {
    plot.states.matrix(apply.reconstruction(trees[[treeNo]], my_chars[, i]),
