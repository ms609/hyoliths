...
if (knitr::is_html_output()) {
  ...
  for (i in eachTree) {
    ...
    analysisBoxes[i] <- paste0('<path d="M', svgWidth -1L, ',', 
                            ...
                            'h-156v14h156z" stroke="#34caaf" fill="#34caafdd"></path>', collapse='')
...
function showTree (tree) {
  $("object.tree").attr("data", str.replace("`r sprintf(fileNamer, '\\d+', '(\\d+)')`",
    "`r sprintf(fileNamer, '" + tree + "', "\\1")`"));
...
#for (i in seq_len(n_char)) {
  ...
  if (knitr::is_html_output()) {
    ...
    for (treeNo in eachTree) {
      ...
      edges <- paste0('\n\t<path d="M', nodeXi[parent[treeNo, eachEdge]],
                      ',', nodeYi[parent[treeNo, eachEdge]],
                      'V', nodeYi[child[treeNo, eachEdge]],
                      'H', nodeXi[child[treeNo, eachEdge]],
                      '" stroke="', edgeCol[eachEdge],
                      LineStyle(edgeCol[eachEdge]), '"></path>')
      tips <- paste0('\n\t<text x="', (nodeXi[terminal] + 2L), '" y="', nodeYi[terminal] + textY,
                     '" fill="', tipCol[terminal],
                     '" class="flag">[', tipFlag[terminal], ']</text>',
                     '\n\t<text x="', (nodeXi[terminal] + 22L), '" y="', nodeYi[terminal] + textY,
                     '" class="taxonLabel">', 
                     gsub('_', ' ', tipLabel[treeNo, terminal], fixed=TRUE), '</text>')
      legendY <- round(rev(svgHeight - (yStep * seq_along(legendLabels))), 1)
      legend <- paste0('\n\t<path d="M0,', legendY, 'h22" stroke="', legendCol,
    ...
    cat(paste0('<object type="image/svg+xml" data="', sprintf(fileNamer, treeNo, i), 
