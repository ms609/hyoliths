...
if (knitr::is_html_output()) {
  ...
  analysisBoxes <- matrix('', n_tree, nAnalyses)
  for (i in eachTree) {
    tree <- trees[[i]]
    ...
    analysisBoxes[i, ] <- paste0('<path d="M', svgWidth -1L, ',', 
                            yStep * 5L + 16L + (seq_len(nAnalyses) * 16L), 
  ...
  cat(paste0('<div id="switcherTemplate" class="switcher"><div class="selectTree">Select tree:</div>',
...
function showTree (tree) {
  $("object.tree").attr("data", 
    $("object.tree").attr("data")
                    .replace(/tree\d+\-char(\d+)/, 
                             "tree" + tree + "-char$1")
  );
...
$(document).ready(function(){
  var originalSwitcher = $("#switcherTemplate");
  $("object.tree").each(function()
    {originalSwitcher.clone().insertBefore($(this));}
  );
...
#for (i in seq_len(n_char)) {
  ...
  if (knitr::is_html_output()) {
    ...
    for (treeNo in eachTree) {
      ...
      write(paste0('<?xml version="1.0" standalone="no"?>\n',
                   ...
                   svgWidth, '" height="', svgHeight, '">',
                    ...
                    ciCaption, 
                    paste0(analysisBoxes[treeNo, analysisNames %in% treeNames[treeIndex==treeNo]], collapse='\n'),
          '\n</svg>'), file=paste0('_book/', sprintf(fileNamer, treeNo, i)))
    }
    ...
    cat(paste0('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="', svgWidth, 
        ...
        paste0('<text x="', svgWidth - 4L,
               ...
               '<tspan dy="16">', 
               paste0(c('A most parsimonious tree under:', analysisNames),
