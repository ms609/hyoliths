...
function switchTree(input) {
  ...
  if (!isNaN(newValue)) {
    ...
    showTree(newValue, $(input).data("char"));
  }
  return false;
}

function updateAllTrees(tree) {
  $my.trees.each(function(index, item) {
    var oldTree = $(item);
    $.ajax({
       //url: "`r sprintf(fileNamer, '" + tree + "', '" + $(oldTree).data("char") + "')`",
       url: "Brachiopod_phylogeny_files/figure-html/tree" + tree + "-char" +
  ...
  enableTaxonHighlighting();
  return false;
}

function showTree(tree, char) {
  var oldTree = $('[char=' + char + ']', $my.trees);
  $.ajax({
     //url: "`r sprintf(fileNamer, '" + tree + "', '" + char + "')`",
    url: "Brachiopod_phylogeny_files/figure-html/tree" + tree + "-char" +
            $(oldTree).data("char") + ".svg"
    context: oldTree,
    datatype: 'xml'
    ...
    $(this).replaceWith($('svg', newTree));
    if (typeof (updateTimeout) != 'undefined') window.clearTimeout(updateTimeout);
    updateTimeout = window.setTimeout(updateAllTrees(tree), 350);
  });
  return false;
}

function enableTaxonHighlighting() {
  $my.trees.click(function (e) {
    var target = e.target; // Which node triggered the click
    $target = $(target);
    if (target.nodeName === 'TEXT') {
      $('text:contains("' + $(this).html() + '")').toggleClass("highlightTaxon");
    }
  })
  return false;
}
...
$(document).ready(function() {
  ...
  $my.trees.each(function()
    {
      var switcherClone = originalSwitcher.clone();
      $('.switcherNumber', switcherClone)data("char", $(this).data("char"));
      switcherClone.insertBefore($(this));
      return false;
    }
  );
...
for (i in seq_len(n_char)) {
  ...
  if (knitr::is_html_output()) {
    for (treeNo in eachTree) {
      ...
      tips <- paste0('\n <text x="', (nodeXi[terminal] + 0L), '" y="', nodeYi[terminal] + textY,
