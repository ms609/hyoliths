...
Alongside each character's definition, each character is mapped onto a tree.
...
```
```{R SVG-setup, echo=FALSE, asis=TRUE}
CheckDirExists <- function (x) if (!dir.exists(x)) dir.create(x)
CheckDirExists('_bookdown_files')
CheckDirExists('_bookdown_files/Brachiopod_phylogeny_files')
CheckDirExists('_bookdown_files/Brachiopod_phylogeny_files/figure_html')
fileNamer <- './_bookdown_files/Brachiopod_phylogeny_files/figure_html/tree%s-char%s.svg'
if (knitr::is_html_output()) {
  nTree <- c(length(ew.trees), vapply(iw.trees, length, 0))
  analysisNames <- c('Equal weights', paste0('Implied weights, k = ', kValues))
  ...
  svgWidth <- 682
  svgHeight <- 595
  treeXMargins <- c(10, 180)
  treeYMargins <- c(12, 80)
  ...
  parent <- child <- matrix(0, n_tree, n_tip)
  eachEdge <- seq_len(dim(tree$edge)[1])
  xStep <- nodeX <- nodeY <- vector('list', n_tree)
  yStep <- round((svgHeight - treeYMargins[2] - treeYMargins[1]) / n_tip, 1)
  textY <- 3L
  analysisBoxes <- character(n_tree)
  for (i in eachTree) {
    tree <- trees[[i]]
    ...
    nodeYi[child[treeNo, child[treeNo, ] <= n_tip]] <- seq(treeYMargins[1], svgHeight-treeYMargins[2], len=n_tip)
    
    analysisBoxes[i] <- paste0('<path d="M', svgWidth -1L, ',', 
                            yStep * 5L + (seq_along(analysisNames) * 16L), 
                            'h-156v14h156z" stroke="#34caaf" fill="#34caafdd"></path>')
...
function showTree (tree) {
  $("img.tree").attr("src", str.replace("`r sprintf(treeNamer, '\\d+', '(\\d+')')`",
    "`r sprintf(treeNamer, '" + tree + "', "\\1")`"));
  $(".switch").css("color", "#34caaf");
  $(".switch" + tree).css("color", "#223");
  $(".switch").css("background-color", "#fcfcfc");
  $(".switch" + tree).css("background-color", "#eee");
}
</script>
...
#for (i in seq_len(n_char)) {
  ...
  if (knitr::is_html_output()) {
    cat(paste0('<div class="switcher"><div class="selectTree">Select tree:</div>',
           paste0('<div class="switch switch', eachTree, 
                  '" onmouseover="showTree(', eachTree, 
                            ');">', eachTree, '</div>', collapse=''),
          '</div>'))
    ...
    for (treeNo in eachTree) {
      ...
      tipCol <- matrix_data$tips_colours
      tipFlag <- matrix_data$tips_labels
      
      
      LineStyle <- function (x) ifelse(x == 'lightgrey', '" class="inapplicable', '')
      edges <- paste0('<path d="M', nodeX[parent[treeNo, eachEdge]], ',', nodeY[parent[treeNo, eachEdge]],
             'V', nodeY[child[treeNo, eachEdge]],
             'H', nodeX[child[treeNo, eachEdge]],
             '" stroke="', edgeCol[eachEdge],
             LineStyle(edgeCol[eachEdge]),
             '"></path>')
      tips <- paste0('<text x="', (nodeX[terminal] + 2L), '" y="', nodeY[terminal] + textY,
                     '" fill="', tipCol[terminal],
                     '" class="flag">[', tipFlag[terminal], ']</text>',
                     '<text x="', (nodeX[terminal] + 22L), '" y="', nodeY[terminal] + textY,
                     '" class="taxonLabel">', 
                     gsub('_', ' ', tipLabel[terminal], fixed=TRUE), '</text>')
      legendY <- round(rev(svgHeight - (yStep * seq_along(legendLabels))), 1)
      legend <- paste0('<path d="M0,', legendY, 'h22" stroke="', legendCol,
                       LineStyle(legendCol), '"></path>',
                       '<text x="30" y="', legendY + textY, '" class="legendLabel">', 
                       legendLabels, '</text>')
      nRegions <- states_matrix$regions
      if (length(nRegions) == 0) nRegions <- 0
      ciCaption <- paste0('<text x="', svgWidth - 4L,
                   ...
                   '<tspan class="score">', states_matrix$regions,
                   '</tspan> additional region', ifelse(states_matrix$regions == 1, '', 's'), 
                   '</text><text x="', svgWidth - 4L,
                   '" y="', (yStep * 3) + textY, '" text-anchor="end" class="stepsLabel">',
                    ci_text[i, treeNo], '</text>',
                   '</text>')
      
      write(paste0('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="', svgWidth, 
          '" height="', svgHeight, '">',
          paste0(tips, collapse=''),
          paste0(edges, collapse=''),
          ciCaption, analysisBoxes[analysisNames %in% treeNames[treeIndex==treeNo]],
          '</svg>'), file=sprintf(fileNamer, treeNo, i))
      cat(paste0('<img src="', sprintf(fileNamer, treeNo, i), 
    ...
    cat(paste0('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="', svgWidth, 
        '" height="', svgHeight, '" class="treeOverlay">',
        paste0(legend, collapse=''),
        paste0('<text x="', svgWidth - 4L,
                   '" y="', (yStep * 5) + textY, '" text-anchor="end" class="stepsLabel">',
                   '<tspan dy="16">', 
                   paste0(c('A most parsimonious tree under:',
                            analysisNames),
                          collapse=paste0('</tspan><tspan x="', svgWidth - 2L, 
                          '" style="font-style:italic" dy="1.2em">')),
                    '</tspan></text>'),
        '</svg>'))
  } else {
