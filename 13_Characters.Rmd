...
function updateAllTrees(tree) {
  $(".tree").each(function(index, item) {
    var oldTree = $(item);
    $.ajax({
       url: "`r sprintf(fileNamer, '" + tree + "', '" + $(oldTree).attr("char") + "')`",
      context: item,
      datatype: 'xml'
...
function enableTaxonHighlighting() {
  $('text.taxonLabel').click(function () {
  ...
  $(".tree").each(function()
    {
      ...
      $('.switcherNumber', switcherClone).attr("char", $(this).attr("char"));
