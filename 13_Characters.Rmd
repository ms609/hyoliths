...
function showTree (tree) {
  ...
  $('svg.tree' + tree).css('display', 'block');
...
#for (i in seq_len(n_char)) {
  ...
  if (knitr::is_html_output()) {
    ...
    for (treeNo in eachTree) {
      ...
      xStep <- round(seq(treeXMargins[1], to=svgWidth-treeXMargins[2],
                         len=max(nAncestors) + 1L), 1)
      nodeX <- xStep[nAncestors + 1L]
      nodeY <- double(length(nAncestors))
      nodeY[child[child <= n_tip]] <- seq(treeYMargins[1], svgHeight-treeYMargins[2], len=n_tip)
      yStep <- round((svgHeight - treeYMargins[2] - treeYMargins[1]) / n_tip, 1)
      
      analysisBoxes <- paste0('<path d="M', svgWidth -1L, ',', 
                              yStep * 5L + (seq_along(analysisNames) * 16L), 
                              'h-156v14h156z" stroke="#34caaf" fill="#34caafdd"></path>')

      for (nodeI in rev(internal)) {
        nodeY[nodeI] <- mean(nodeY[Children(tree, nodeI)])
      }
      nodeY <- round(nodeY, 1)
      
      LineStyle <- function (x) ifelse(x == 'lightgrey', '" class="inapplicable', '')
      textY <- 3L
      edges <- paste0('<path d="M', nodeX[parent[eachEdge]], ',', nodeY[parent[eachEdge]],
             'V', nodeY[child[eachEdge]],
             'H', nodeX[child[eachEdge]],
      ...
      cat(paste0('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="', svgWidth, 
          '" height="', svgHeight, ifelse(treeNo==1, '', '" display="none'),
          '" class="tree tree', treeNo, '">',
