...
#for (i in seq_len(n_char)) {
  ...
  if (knitr::is_html_output()) {
    ...
    cat(paste0('>  <svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="', svgWidth, 
        '" height="', lineHeight * length(legendLabels), '" class="statesLegend">',
        paste0('\n <path d="M0,', legendY - 5L, 'h22" stroke="', legendCol,
                       LineStyle(legendCol), '"></path>',
                       '\n <text x="30" y="0" class="legendLabel"><tspan dy="',
                       legendY, '">', 
                       legendLabels, '</tspan></text>', collapse=''),
        paste0('<text x="', svgWidth - 4L,
               '" y="', (yStep * 5) + textY, '" text-anchor="end" class="stepsLabel">',
               '</text>'),
        '</svg> \n>\n'))
    cat("> ", if (IsTransformational(my_states[[i]])) "Transformational" else "Neomorphic", "character.  \n>\n")
  } else {
