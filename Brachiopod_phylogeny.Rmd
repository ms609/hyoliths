...
```{R "Load IW trees TreeSearch", cache=TRUE}
...
```{r, fig.cap="Consensus of implied weights analyses at all values of k", fig.width=6, fig.height=5, cache=TRUE}
...
```{r, fig.cap="Implied weights results", fig.width=7.1, fig.height=10.5, cache=TRUE}
...
```{r "equal weights results in TreeSearch", fig.cap="Strict consensus of equal weights results", cache=TRUE}
...
```{R, fig.cap="Strict consensus of equal weights results, taxa excluded", cache=TRUE}
omit <- c(lon)
...
```{R load_character_data, echo=FALSE, cache=TRUE}
library('Inapp')
...
PrintDivision <- function (parts) cat(Header(2L, paste(char_parts_i[1:parts],
                                                       collapse=': ')))
PrintCharacter <- function () cat(Header(3L, paste0("[", i, "] ",
                                           char_parts_i[char_nesting[i]], " {-}")))
PrintSoloCharacter <- function () cat(Header(2L, paste0(char_parts_i, " [", i, "] ")))
...
for (i in seq_len(n_char)) {
  ...
  if (char_nesting[i] == 1 && 
    ...
    PrintCharacter()
      ...
      PrintDivision(1)
      PrintCharacter()
  } else if (length(char_parts_i) > 2L) {
    if (length(char_parts_last) < 2L
        || char_parts_last[2L] != char_parts_i[2L]) {
      PrintDivision(2)
    }
    PrintCharacter()
  } else {
