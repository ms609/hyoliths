...
ew.tree <- Ratchet(start.tree, my_data, verbosity=0L,
                   ...
                   swappers=list(RootedTBRSwap, RootedSPRSwap, RootedNNISwap))
write.nexus(ew.tree, file=paste0("TreeSearch/hy_ew_", Fitch(ew.tree, my_data), ".nex", collapse=''))
...
omit=c(lon, 'Clupeafumosus_socialis', 'Tomteluva_perturbata',
       'Yuganotheca_elegans', 'Mummpikia_nuda',
...
```{R, fig.cap="Strict consensus of equal weights results, taxa excluded", eval=FALSE}
# omit <- c("none")
...
```{R char_loading_functions, echo=FALSE}
...
ConvertReferences <- function (string) {
  ...
  ReplaceRefs <- function (pattern, strings, Replace) {
    ...
    keys <- lapply(matchText, function (x) unique(gsub(pattern, "~~~\\1\\3\\4~~~",
                                                       x, perl=TRUE)))
...
```
```{R load_the_characters, echo=FALSE}
library('Inapp')
#tree <- iw.trees[[3]][[1]]
tree <- ew.tree
...
> mcmcp ngen=1000000 samplefreq=500 nruns=2 nchains=8 burninfrac=0.1;
