...
(ref:tnt-maj-cons) Majority-rule consensus of all trees that are optimal under $3 \leq k \leq 24$.

(ref:tnt-node-freq) Each node is labelled with its frequency in the set of all optimal trees under all examined values of $k$, where less than 100%.
...
```{R tnt-iw-consensus, fig.wid=7.1, fig.height=8, fig.cap="(ref:tnt-iw-results) (ref:first-panels) (ref:tnt-jacks) (ref:tnt-iw-explanation)", echo=FALSE}
par(mfrow=c(1, 2))
...
```
```{R tnt-iw-7-24, fig.width=7.1, fig.height=8, fig.cap="(ref:tnt-iw-results), (ref:third-panels)", echo=FALSE}
par(mfrow=c(1, 2))
...
(ref:tnt-ew-cons) Consensus of all trees obtained using equally weighted Fitch parsimony in TNT.  
