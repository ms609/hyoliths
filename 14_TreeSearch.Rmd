...
`r GitLink("14_TreeSearch.Rmd", "TreeSearch.Rmd")`.  The complete analysis will take several hours.
...
ew.tree <- Ratchet(start.tree, my_data, verbosity=0L,
                   ratchHits = 20, ratchIter=4000, searchHits=55, # ratchHits = 20 not enough
                   swappers=list(RootedTBRSwap, RootedSPRSwap, RootedNNISwap))
...
# Deeper search with IW, as default parameters have not always hit the globally best trees
source('loadTrees.R')
for (k in kValues) {
...
```{r treesearch-iw-indiv-1, fig.width=7.1, fig.height=8, fig.cap="(ref:ts-iw-indiv) (ref:first-panels)", echo=FALSE}
...
```{R treesearch-iw-indiv-2, fig.width=7.1, fig.height=8, fig.cap="(ref:ts-iw-indiv) (ref:second-panels)", echo=FALSE}
par(mfrow=c(1, 2), mar=rep(0.2, 4))
...
```{R treesearch-iw-indiv-3, fig.width=7.1, fig.height=8, fig.cap="(ref:ts-iw-indiv) (ref:third-panels)", echo=FALSE}
par(mfrow=c(1, 2), mar=rep(0.2, 4))
