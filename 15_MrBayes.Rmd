...
```{R mrbayes-full-consensus, echo=FALSE, fig.cap="Results of Bayesian analysis, posterior probability > 50%, all taxa"}
omit <- c("none")
...
```{R mrbayes-pruned-consensus, echo=FALSE, fig.cap="Results of Bayesian analysis, posterior probability > 50%, wildcard taxa pruned"}
omit <- c(
...
(ref:bayes-pstat) Parameter estimates. Convergence was indicated by PSRF = 1.00 and an estimated sample size of > 200 for each parameter.
...
knitr::kable(pstat, caption="(ref:bayes-pstat)")
```
