...
if (knitr::is_html_output()) {
  ...
  majLabels <- vapply(conTip + 1:Nnode(majCon), function (node) {
    where <- vapply(recons, function (char) vapply(char, function (x) node %in% whichNode[x - conTip], logical(1)), logical(3))
  ...
  conCaption <- paste0('<g transform="translate(', svgWidth - 4L, ' 0)">',
                       ...
                       'Character transformations</tspan>', 
                       '<tspan x="0" dy="1.2em"><tspan class="score b42">Orange</tspan>: ',
                       'Additional regions</tspan>', 
  ...
  majCaptions <- vapply(conTip + 1:Nnode(majCon), function (node) {
    ...
    apply(where, 1, function (changes) {
      paste0('<span class=change><span class=charNo>', which(changes), '</span>: ',
             names(changes[changes]), '</span>', collapse='')
    })
  }, character(3))
  cat('<div id=nodeDetails>Click a node for details of changes</div>',
      paste0('<div id=nodeDetail', internal, ' class=nddtl><span class=nddtlhdr>',
         'Steps incurred at node ', internal, '</span><div class=tfm>',
         majCaptions['changes', internal - conTip], '</div><div class=rgn>',
         majCaptions['regions', internal - conTip], '</div><div class=ftch>',
         majCaptions['fitches', internal - conTip], '</div></div>', collapse=''))
} else {
  ColPlot(majCon)
  majLabels <- vapply(conTip + 1:Nnode(majCon), function (node) {
  where <- vapply(recons, function (char) vapply(char, function (x) node %in% whichNode[x - conTip], logical(1)), logical(3))
  whichWhere <- apply(where, 1, which)
  # Return: 
  if (length(whichWhere) == 0) character(3) else {
