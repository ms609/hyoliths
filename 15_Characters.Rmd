...
recons <- apply(maj_chars, 2, function (char) {
...
nTip <- length(majTips)
majSplits <- vapply(Descendants(majCon, nTip + 1:Nnode(majCon), type='tips'),
           function (tips) majTips %in% majTips[tips], logical(nTip))
bifSplits <- vapply(Descendants(bifurcating, nTip + seq_len(nTip - 1L), type='tips'),
           function (tips) bifTips %in% bifTips[tips], logical(nTip))
...
whichNode <- whichNode + nTip
ColPlot(majCon)
majLabels <- vapply(nTip + 1:Nnode(majCon), function (node) {
  where <- vapply(recons, function (char) vapply(char, function (x) node %in% whichNode[x - nTip], logical(1)), logical(3))
