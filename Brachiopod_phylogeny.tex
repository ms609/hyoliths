...
\hypersetup{unicode=true,
            pdftitle={Supplementary Information for: Hyoliths with pedicles illuminate the origin of the brachiopod body plan},
...
\href{https://morphobank.org/permalink/?P2800}{project 2800}; a static
version can be downloaded directly :
...
Characters are coded following the
\href{https://rawgit.com/TGuillerme/Inapp/master/inst/gitbook/_book/}{recommendations}
of \citet{Brazeau2018}:

\begin{itemize}
\item
  We have employed reductive coding \citep{Wilkinson1995}, using a
  distinct state to mark character inapplicability. Character
  specifications follow the structural syntax of \citet{Sereno2007} in
  order to highlight ontological dependence between characters and
  emphasize the structure of the dataset.
\item
...
\item
  Saving
  \href{https://raw.githubusercontent.com/ms609/hyoliths/master/tnt.run}{raw.githubusercontent.com/ms609/hyoliths/master/tnt.run}
  to the same directory, with the filename \texttt{tnt.run}.
\item
...
Optimal trees can be downloaded in TNT Newick format from
\href{https://github.com/ms609/hyoliths/tree/master/TNT}{github.com/ms609/hyoliths/tree/master/TNT}.
...
of 12150 total cells), which are known to introduce error and bias to
...
Optimal trees can be downloaded in Nexus format from
\href{https://github.com/ms609/hyoliths/tree/master/TreeSearch}{github.com/ms609/hyoliths/tree/master/TreeSearch}.
...
The posterior tree distribution can be downloaded in Nexus format from
\href{https://github.com/ms609/hyoliths/tree/master/MrBayes}{github.com/ms609/hyoliths/tree/master/MrBayes}.
...
of this document at
\href{https://ms609.github.io/hyoliths/reconstructions.html}{ms609.github.io/hyoliths/reconstructions.html}.
...
\emph{Novocrania}: Shell not secreted until after metamorphosis (Popov
\emph{et al}. 2010). Freeman \& Lundelius (1999) report a
\emph{Craniops}-like larval shell in fossil ``Crania'', but observe that
Quaternary {[}Novo{]}crania no longer exhibit a larval shell.
...
The brephic shell is the shell possessed by the young organism (see
Ushatinskaya \& Korovnikov 2016R and references therein for discussion
of terminology).

\emph{Micrina} resembles linguliforms (Holmer \emph{et al}. 2011): in
...
brephic shells (Holmer \emph{et al} 2011), resembling the shape of a
ship's propeller.
...
\emph{Askepasma toddense}: Renoid -- see fig. 4B3 in Topper \emph{et
al}. 2013T.
...
\emph{et al}. 2013R; cf.~Holmer \emph{et al}. 2011).

\hypertarget{Conotheca-coding-2}{}
\emph{Conotheca}: \citep{Wrona2003}.

\hypertarget{Coolinia_pecten-coding-2}{}
\emph{Coolinia pecten}: See fig. 3 in Bassett \emph{et al}. 2017.
...
\emph{Lingula}: See fig. 159 in Williams \emph{et al}. 1997.
...
``larval shell'' (Bassett \& Popov 2017).
...
\emph{et al}. (2000) tables 5--6.

\hypertarget{Askepasma_toddense-coding-4}{}
\emph{Askepasma toddense}: Indented with hexagonal pits (Williams
\emph{et al}. 1998T, appendix 2).

\hypertarget{Clupeafumosus_socialis-coding-4}{}
\emph{Clupeafumosus socialis}: ``Larval shells on both valves
{[}\ldots{}{]} are covered by fine, shallow pits'' -- Topper \emph{et
al}. 2013R.
...
\emph{Eoobolus}: Pitted (Williams \emph{et al}. 2000, table 8).

\hypertarget{Lingula-coding-4}{}
\emph{Pelagodiscus atlanticus}, \emph{Lingula}: Smooth, following
family-level codings of Williams \emph{et al}. 2000, table 6.
...
\emph{Micrina}: Smooth (Holmer \emph{et al}. 2011F).

\hypertarget{Micromitra-coding-4}{}
\emph{Micromitra}: Pustolose in Paterinidae (Williams \emph{et al}.
2000, table 6).

\hypertarget{Paterimitra-coding-4}{}
\emph{Paterimitra}: Polygonal texture present (Holmer \emph{et al}.
2011F), as in the adult shell.
...
\emph{Salanygolina}: Smooth (Holmer \emph{et al}. 2009T).

\hypertarget{Siphonobolus_priscus-coding-4}{}
\emph{Siphonobolus priscus}: ``Smooth brephic shell'' -- Popov \emph{et
al}. 2009.
...
attachment rudiment in fig. 2 of Balthasar 2009T.
...
thus non-attached) larvae (Popov \emph{et al}. 2009).
...
linguliforms (Holmer \emph{et al}. 2011).
...
\emph{Botsfordia}: A single pair of low tubercles are (Ushatinskaya \&
Korovnikov 2016R state ``may be'') located in the middle region of the
...
\emph{Novocrania}, \emph{Pelagodiscus atlanticus}: Three pairs (Carlson
1995).
...
ventral valve and two in dorsal valve'' (Williams \emph{et al}. 2000) --
if these correspond to setal sacs, then we interpret this as equivalent
to one pair.
...
\emph{Mickwitzia muralensis}: See fig. 2 in Balthasar 2009T.
...
\emph{Siphonobolus priscus}: Two pairs of setal sacs (Popov \emph{et
al}. 2009).
...
preserved along anterior and anterolateral margins (Hu \emph{et al}.
2010).
...
\emph{Eccentrotheca}: Skovsted \emph{et al} (2011) assumed the setae may
have been present along the margin of the adapical opening, but there is
no fossil evidence.

\hypertarget{Heliomedusa_orienta-coding-16}{}
\emph{Heliomedusa orienta}: Throughout the shell -- see Williams
\emph{et al}. 2007 -- causing the pustulose appearance remarked upon by
Chen \emph{et al}. 2007.
...
portion of the ventral mantle. For discussion see Holmer \emph{et al}.
2018T and Bassett and Popov 2017.
...
``The putative pedicle illustrated by Chen \emph{et al}. (2007: Figs 4,
6, 7) in fact is the mold of a three-dimensionally preserved visceral
cavity'' -- Zhang \emph{et al}. 2009.
...
in \emph{Kutorgina} (Holmer \emph{et al}. 2018E; Holmer \emph{et al}.
2018T).
...
posterior opening'' -- Skovsted \emph{et al}. 2009.
...
stacked, three dimensionally preserved, tabular discs'' (Holmer \emph{et
al}. 2018E).\\
This contrasts with the uniform (`massive') pedicles of living taxa.

\hypertarget{Antigonambonites_planus-coding-27}{}
\emph{Antigonambonites planus}: Biomineralized (Holmer \emph{et al}.
2018E).
...
\emph{Acanthotretella spinosa}: Holmer and Caron (2006) interpret the
presence of a bulb as tentative; we score it as ambiguous.
...
Observed in \emph{Pedunculotheca} and \emph{Bethia} (Sutton \emph{et al}
2005).
...
Holmer \emph{et al}. (2018T) remark that the tapering aspect of the
...
many other taxa (\emph{Eichwaldia}, \emph{Bethia}) in which the pedicle
...
with which the pedicle is deformed (Holmer \emph{et al}. 2018E), but its
presence is not known for certain so is coded ambiguous.
...
distally tapering'' -- Holmer \emph{et al}. 2018E.
...
shell. See character 28 in Williams \emph{et al}. (1998T) appendix 1.
Care must be taken not to code an impression as absent when the
preservational quality is insufficient to safely infer a genuine
absence. Treated as neomorphic as the presence of an innervation is
considered a derived state.

\hypertarget{Alisina-coding-34}{}
\emph{Alisina}: Not described by Williams \emph{et al}. 2000.
...
Williams \emph{et al}. 1998T, appendix 2.

\hypertarget{Botsfordia-coding-34}{}
\emph{Botsfordia}: Documented by Skovsted \emph{et al}. 2017.

\hypertarget{Clupeafumosus_socialis-coding-34}{}
\emph{Clupeafumosus socialis}: Coded as absent in Acrotretidae (Williams
\emph{et al}. 2000, table 6).

\hypertarget{Lingula-coding-34}{}
\emph{Lingula}: Present in many lingulids (Williams \emph{et al}. 2000),
and coded as present in Lingulidae (Williams \emph{et al}. 2000, table
6).

\hypertarget{Lingulellotreta_malongensis-coding-34}{}
\emph{Lingulellotreta malongensis}: Coded as present in
Lingulellotretidae (Williams \emph{et al}. 2000, table 6).

\hypertarget{Mummpikia_nuda-coding-34}{}
\emph{Mummpikia nuda}: Balthasar (2008, p.~274) identifies a canal as a
probable impression of a pedicle nerve.

\hypertarget{Nisusia_sulcata-coding-34}{}
\emph{Nisusia sulcata}, \emph{Orthis}: Not reported in Williams \emph{et
al}. 2000.

\hypertarget{Pelagodiscus_atlanticus-coding-34}{}
\emph{Pelagodiscus atlanticus}: Coded as present in Discinidae (Williams
\emph{et al}. 2000, table 6).

\hypertarget{Siphonobolus_priscus-coding-34}{}
\emph{Siphonobolus priscus}: Coded as absent in Siphonotretidae
(Williams \emph{et al}. 2000, table 6).
...
character (contra Williams \emph{et al}. 2000).

For a description of terms see Williams \emph{et al}. (1997, 2000).
...
(Williams \emph{et al}. 1997 p.~418)\\
...
\emph{Antigonambonites planus}: Not reported in Treatise (Williams
\emph{et al}. 2000).

\hypertarget{Askepasma_toddense-coding-36}{}
\emph{Askepasma toddense}: Described as pinnate (at least in ventral
valve) by Williams \emph{et al}. (1998T, p.~250).

\hypertarget{Botsfordia-coding-36}{}
\emph{Botsfordia}, \emph{Eoobolus}: Following Williams \emph{et al}.
1998T, appendix 2, and Williams \emph{et al} (2000), table 8.

\hypertarget{Clupeafumosus_socialis-coding-36}{}
\emph{Clupeafumosus socialis}: Following Table 8 (for Acrotreta) in
Williams \emph{et al}. (2000), and the general pinnate condition for
acrotretoids stated in Williams \emph{et al}. (1997), p.~420.

\hypertarget{Coolinia_pecten-coding-36}{}
\emph{Coolinia pecten}: Not reported in Williams \emph{et al}. 2000.

\hypertarget{Craniops-coding-36}{}
\emph{Craniops}: Not reported from fossil material.

\hypertarget{Gasconsia-coding-36}{}
\emph{Gasconsia}: Williams \emph{et al}. (2000, table 15) appear to use
Palaeotrimerella (as drawn in Williams \emph{et al}. 1997) as a model
for \emph{Gasconsia}, which pre-supposes a close relationship. We are
not aware of any report of mantle canals from \emph{Gasconsia} itself.
...
\emph{Siphonobolus priscus}: Interpreted as baculate, following Havlicek
1982.
...
\emph{genitalia}'' -- Williams \emph{et al}. 1997.

\hypertarget{Tomteluva_perturbata-coding-36}{}
\emph{Tomteluva perturbata}: Preservation not adequate to evaluate
(Streng 2016).

\subsection*{{[}37{]} vascula lateralia}\label{vascula-lateralia}
\addcontentsline{toc}{subsection}{{[}37{]} vascula lateralia}
...
Williams \emph{et al} (1997) write: ``The mantle canal system of most of
the organophosphate-shelled species consists of a single pair of main
trunks in the ventral mantle (\emph{vascula} \emph{lateralia}) and two
pairs in the dorsal mantle, one pair (\emph{vascula} \emph{lateralia})
occupying a similar position to the single pair in the ventral mantle
and a second pair projecting from the body cavity near the midline of
the valve. This latter pair may be termed the \emph{vascula}
\emph{media}, but whether they are strictly homologous with the
\emph{vascula} \emph{media} of articulated brachiopods is a matter of
opinion. It is also impossible to assert that the \emph{vascula}
\emph{lateralia} are the homologues of the \emph{vascula} \emph{myaria}
or \emph{genitalia} of articulated species, although they are likely to
be so as they arise in a comparable position.''
...
presence in Siphonotreta) in Williams \emph{et al}. (2000).

\hypertarget{Alisina-coding-37}{}
\emph{Alisina}, \emph{Kutorgina chengjiangensis}, \emph{Nisusia
sulcata}: Following table 15 in Williams \emph{et al}. (2000).

\hypertarget{Askepasma_toddense-coding-37}{}
\emph{Askepasma toddense}, \emph{Micromitra}: ``Laurie (1987) has shown
that arcuate \emph{vascula} \emph{media} were present in the mantles of
both valves as were pouchlike \emph{vascula} \emph{genitalia},
especially in the ventral valve'' -- Williams \emph{et al}. 1997.

\hypertarget{Botsfordia-coding-37}{}
\emph{Botsfordia}: Following Popov (1992).

\hypertarget{Clupeafumosus_socialis-coding-37}{}
\emph{Clupeafumosus socialis}: Presence indicated in Table 8 (for
Acrotreta) in Williams \emph{et al}. (2000).
...
\emph{Heliomedusa orienta}: Present: Williams \emph{et al}. (2000); Jin
\& Wang (1992).

\hypertarget{Lingulellotreta_malongensis-coding-37}{}
\emph{Lingulellotreta malongensis}: Present (Williams \emph{et al}.
2000).
...
\emph{lateralia}'' -- Zhang \emph{et al} 2007A.

\hypertarget{Novocrania-coding-37}{}
\emph{Novocrania}: Following table 15 in Williams \emph{et al}. (2000)
(for \emph{Neocrania}), who write that ``Holocene craniides have only a
single pair of main trunks in both valves, corresponding to the
\emph{vascula} \emph{lateralia}''. Williams \emph{et al}. (2007)
reiterate this position (p.~2875), at least for the ventral valve.
...
\emph{et al}. 2014 (and supplementary material), the mantle canals are
interpreted as lateral, with no clear \emph{vascula} \emph{media}
present.

\subsection*{{[}38{]} vascula media}\label{vascula-media}
\addcontentsline{toc}{subsection}{{[}38{]} vascula media}
...
Williams \emph{et al}. (1997) note that in addition to the
\emph{vascula} \emph{lateralia}, ``\emph{Discinisca} has two additional
mantle canals emanating from the body cavity into the dorsal mantle
(\emph{vascula} \emph{media}).''
...
\emph{Botsfordia}: Following Popov (1992, fig. 2).
...
\emph{Eoobolus}: Fig. 5 in Balthasar 2009T.
...
\emph{Glyptoria}: Present and divergent (Williams \emph{et al}. 2000).

\hypertarget{Heliomedusa_orienta-coding-38}{}
\emph{Heliomedusa orienta}: Present: Williams \emph{et al}. (2000) p162,
Jin \& Wang (1992).
...
\emph{Longtancunella chengjiangensis}: Reported by Zhang \emph{et al}.
(2007A) though the interpretation is tentative.

\hypertarget{Micromitra-coding-38}{}
\emph{Micromitra}: Reported by Williams \emph{et al}. (1998T).

\hypertarget{Novocrania-coding-38}{}
\emph{Novocrania}: Williams \emph{et al}. (2000) write ``Holocene
...
But in contrast, Williams \emph{et al}. 2007, p.~2875, identify the
...
\emph{Terebratulina}: ``In modern terebratulides, the \emph{vascula}
\emph{media} are subordinate to the lemniscate or pinnate \emph{vascula}
\emph{genitalia}'' -- Williams \emph{et al}. 1997 p417.
...
\subsection*{{[}39{]} vascula terminalia}\label{vascula-terminalia}
\addcontentsline{toc}{subsection}{{[}39{]} vascula terminalia}
...
Presumed to be connected with setal follicles in life (Williams \emph{et
al}. 1998T). See Williams \emph{et al}. (2000) for discussion.

\hypertarget{Acanthotretella_spinosa-coding-39}{}
\emph{Acanthotretella spinosa}: Preservation not clear enough to score
with certainty (Holmer \& Caron 2006).

\hypertarget{Alisina-coding-39}{}
\emph{Alisina}: Interomedial \emph{vascula} \emph{terminalia} not
reported by Williams \emph{et al}. (2000).

\hypertarget{Askepasma_toddense-coding-39}{}
\emph{Askepasma toddense}, \emph{Micromitra}: Peripheral only (Williams
\emph{et al}. 1998T; Williams \emph{et al}. 2000).
...
in appendix 2 in Williams \emph{et al}. (1998T).

\hypertarget{Lingula-coding-39}{}
\emph{Lingula}: Peripheral and medial for all Lingulata (Williams
\emph{et al}. 2000).
...
see fig. 1.A1 in Balthasar \& Butterfield 2009E.

\hypertarget{Novocrania-coding-39}{}
\emph{Novocrania}: Peripheral only (Williams \emph{et al}. 2000, p.158).

\hypertarget{Orthis-coding-39}{}
\emph{Orthis}: See schematics in Williams \emph{et al}. (2000).
...
\emph{Siphonobolus priscus}: Not reported in Havlicek 1982 or Williams
\emph{et al}. 2000.
...
slender flexible tentacles'' -- Zhang \emph{et al}. 2009.

\hypertarget{Kutorgina_chengjiangensis-coding-42}{}
\emph{Kutorgina chengjiangensis}: Tentacles ``cannot be confidently
demonstrated in the available specimens.'' -- Zhang \emph{et al}. 2007R.
...
arranged into an ablabial and adlabial row, with ablabial tentacles
...
appendix 1, character 37. Also states in Williams \emph{et al}. 2000,
p.~158.
...
After Carlson (1995), character 37. Lophophore tentacles are commonly
arranged into an ablabial and adlabial row, with ablabial tentacles
sometimes added later in development (and thus interpreted as a
neomorphic addition).
...
extant lingulid brachiopods'' -- Zhang \emph{et al}. 2009.
...
in Carlson (1995), appendix 1, character 37.

\hypertarget{Lingulellotreta_malongensis-coding-44}{}
\emph{Lingulellotreta malongensis}: Single palisade (Zhang \emph{et al}.
2004N).
...
tentacle early in development that is lost during ontogeny.
...
diverge laterally (Moysiuk \emph{et al} 2017).
...
posteriad, rather than anteriad as in lingulids. See Zhang \emph{et al}.
2009; Moysiuk \emph{et al}. 2017.
...
anterior to the body cavity'' -- Zhang \emph{et al}. 2009.
...
\emph{Novocrania}, \emph{Pelagodiscus atlanticus}, \emph{Lingula},
\emph{Terebratulina}, \emph{Phoronis}: Following coding for higher taxon
in Carlson (1995), appendix 1, character 55.
...
the lophophore (Moysiuk \emph{et al}. 2017). This is considered as
...
vase-shaped visceral cavity reported by Zhang \emph{et al}. 2009.

\hypertarget{Lingulellotreta_malongensis-coding-54}{}
\emph{Lingulellotreta malongensis}: An anterior projection of the
visceral area is noted by Williams \emph{et al}. (2000) and considered
equivalent to that observed in \emph{Lingulosacculus} (Balthasar \&
Butterfield 2009E).

\hypertarget{Lingulosacculus-coding-54}{}
\emph{Lingulosacculus}: The prominent anterior extension of the visceral
area noted by Balthasar \& Butterfield (2009E) is considered as
potentially homologous with that of \emph{Heliomedusa} (Zhang \emph{et
al}. 2009) and, by extension, \emph{Haplophrentis} (Moysiuk \emph{et
al}. 2017).
...
of mouth (see fig. 2c, d in Zhang \emph{et al}. 2014).
...
blind-ending intestine'' -- Zhang \emph{et al}. 2007R, p.~1399.
...
mouth'' -- Williams \emph{et al}. 2007, p.~2884\\
...
position.'' -- Williams \emph{et al}. 2007, p.\\
2884.
...
3, and extended data 5 in Moysiuk \emph{et al}. (2017). The text states
in error that the anus is to the left of the midline.
...
exits the posterior extension of the ventral valve'' (Balthasar \&
Butterfield 2009E, p.310).
...
elements, such as the L-elements of \emph{Paterimitra} (Skovsted
\emph{et al}. 2015). Hyolithid helens are coded as potentially
...
\emph{Paterimitra}: L-sclerites (Skovsted \emph{et al}. 2009T).
...
strophic in Williams \emph{et al} (1998T).
...
\emph{Craniops}: Astrophic: rounded posterior margin (see fig. 91 in
Williams \emph{et al}. 2000).
...
\emph{Eoobolus}: Coded as astrophic in Williams \emph{et al} (1998T);
see also \citet{Balthasar2009Thebrachiopod}.
...
\emph{et al} (2001) argue for an astropic interpretation -- whilst
noting that the arrangement is prominently different from other
astrophic taxa. We therefore code this taxon as ambiguous.
...
Zhang \emph{et al}. 2011T.

\hypertarget{Micrina-coding-71}{}
\emph{Micrina}: Non-strophic: see Holmer \emph{et al} 2008.
...
those of other rhynchonelliforms'' (Williams \emph{et al}. 2000,
p.~208).

\hypertarget{Novocrania-coding-71}{}
\emph{Novocrania}: Craniides have a strophic posterior valve edge
(Williams \emph{et al}. 2007, table 39 on p.~2853): \emph{Novocrania}'s
``dorsal posterior margin'' is ``straight'' (Williams \emph{et al}.
2000, p.~171).

\hypertarget{Salanygolina-coding-71}{}
\emph{Salanygolina}: Coded as strophic in Williams \emph{et al} (1998T);
see \citet{Holmer2009Theenigmatic}.
...
weakly to moderately sulcate'' (Topper \emph{et al}. 2013T); a similar
description is provided by Williams \emph{et al}. (2000). Coded as
ambiguous for these two states accordingly.
...
reported by Topper \emph{et al}. (2013R).
...
of the ventral interarea'' -- Williams \emph{et al}. 2000, p.184,
concerning order Trimerellida.
...
ridges on the inner sides'' (Williams \emph{et al} 2000, p.~211); this
simple hinge mechanism is different from other rhynchonelliforms
(Williams \emph{et al}. 2000, p.208), but serves an equivalent purpose
and is thus potentially homologous. We thus code kutorginids as present,
using a subsequent character to capture difference in tooth morphology.
...
Family Obolellidae in Williams \emph{et al}. (2000) notes that
...
articulation structures such as teeth and sockets (Streng \emph{et al}.
2016).
...
resorption of the secondary shell (fig. 322 in Williams \emph{et al}
1997).
...
bearing oblique ridges on the inner sides'' (Williams \emph{et al} 2000,
p.~211); this simple hinge mechanism is different from other
rhynchonelliforms (Williams \emph{et al}. 2000, p.208; table 13
character 30), and is described as a ``pseudodont articulation'' (Holmer
\emph{et al}. 2018E).
...
the inner sides'' -- Williams \emph{et al} 2000, p.~211.
...
dorsal interarea (Holmer \emph{et al}. 2018E). (Coded as ``deltidiodont
teeth absent'' in Benedetto (2009).).
...
Williams \emph{et al}. 1997 (p362) write: ``Teeth {[}\ldots{}{]} are
commonly supported by a pair of variably disposed plates also built up
...
Williams \emph{et al}. 2000 (p.201) write: ``The denticles lack
supporting structures in all Obolellida, but in Naukatida they are
supported by an arcuate plate below the\\
...
mechanism (see Williams \emph{et al}. 2000, p.~184, for details of the
articulation).
...
\emph{Eoorthis}) in Benedetto (2009).
...
Simplified from Bassett \emph{et al}. (2001) character 16.\\
...
See figs 323ff in Williams \emph{et al}. (1997).
...
\emph{Gasconsia}: ``Articulatory structure comprising ventral cardinal
socket and dorsal hinge plate'' -- Williams \emph{et al}. 2000, p.~184.
...
\emph{Ussunia}: Following table 15 in Williams \emph{et al}. 2000.
...
After Bassett \emph{et al}. (2001) character 17. May be difficult to
distinguish from a brachiophore (see Fig 323 in Williams \emph{et al}
1997), so the two structures are not distinguished here.
...
\emph{Micrina}: Prominent ventral muscle scars -- see e.g.~Holmer
\emph{et al} 2008, fig. 1f.
...
between, the \emph{vascula} \emph{lateralia} (Popov 1992).

\hypertarget{Acanthotretella_spinosa-coding-83}{}
\emph{Acanthotretella spinosa}: ``Individual muscle scars cannot be
distinguished'' -- Holmer \& Caron 2006.

\hypertarget{Alisina-coding-83}{}
\emph{Alisina}: Following reconstruction of Gorjansky \& Popov (1986).
...
illustrated in Popov (1992).

\hypertarget{Craniops-coding-83}{}
\emph{Craniops}: See fig. 89 in Williams \emph{et al}. (2000).

\hypertarget{Eoobolus-coding-83}{}
\emph{Eoobolus}: The `laterals' of Balthasar (2009T, fig. 5) are
situated almost upon the \emph{vascula} \emph{lateralia}; they are
interpreted as sitting posterolateral to them.

\hypertarget{Gasconsia-coding-83}{}
\emph{Gasconsia}: Musculature described in Hanken \& Harper (1985), but
location of mantle canals unknown.

\hypertarget{Glyptoria-coding-83}{}
\emph{Glyptoria}: Posterolateral reflected by diductor attachments; see
fig. 18.3.2 in Bassett \emph{et al}. 2001.

\hypertarget{Kutorgina_chengjiangensis-coding-83}{}
\emph{Kutorgina chengjiangensis}: Following situation in \emph{Nisusia};
see fig. 18.2 in Bassett \emph{et al}. (2001).

\hypertarget{Lingulellotreta_malongensis-coding-83}{}
\emph{Lingulellotreta malongensis}: See fig. 5 in Holmer \emph{et al}.
1997E.

\hypertarget{Micromitra-coding-83}{}
\emph{Micromitra}: Posteriomedial muscle field (Williams \emph{et al}.
1998T, text-fig. 6) treated as equivalent to posterolateral muscles.

\hypertarget{Nisusia_sulcata-coding-83}{}
\emph{Nisusia sulcata}: Posterolateral diductors (fig. 18.2 in Bassett
\emph{et al}. 2001).

\hypertarget{Novocrania-coding-83}{}
\emph{Novocrania}: Posterior adductor muscles attach posterolaterally to
ventral mantle canal (Robinson 2014).
...
directly equivalent to the canonical; see. fig 6b in Balthasar (2009T).

\hypertarget{Salanygolina-coding-83}{}
\emph{Salanygolina}: Ventral musculature not clearly constrained (Holmer
\emph{et al}. 2009T).

\hypertarget{Siphonobolus_priscus-coding-83}{}
\emph{Siphonobolus priscus}: Coded following general siphonotretid
condition described by Popov (1992, p.~407).
...
After Bassett \emph{et al}. (2001) character 7.\\
...
\emph{Alisina}: Muscle scars scored based on \emph{Alisina}
\emph{comleyensis} (Bassett \emph{et al}. 2001). The presence of an
adjustor is marked as not presently available, as it is not clear that a
...
musculature presented by Williams \emph{et al}. (2000), fig. 81.

\hypertarget{Botsfordia-coding-84}{}
\emph{Botsfordia}: Not described in Popov (1992).

\hypertarget{Clupeafumosus_socialis-coding-84}{}
\emph{Clupeafumosus socialis}: Not known in any acrotretid (Williams
\emph{et al}. 2000); not evident in \emph{Clupeafumosus} (Topper
\emph{et al}. 2013R).

\hypertarget{Gasconsia-coding-84}{}
\emph{Gasconsia}: No mention of an adjustor muscle in \emph{Gasconsia}
or Trimerellida more generally on pp.~184--185 of Williams \emph{et al}.
2000, nor in discussion in Williams \emph{et al}. 2007 (p.~2850). Coded
as absent.
...
(Williams \emph{et al}. 2000; Popov \emph{et al}. 2009).
...
After Bassett \emph{et al}. (2001) character 8, and Williams \emph{et
al}. (1996, character 35; 2000, p.~160, character 54)
...
(Williams \emph{et al}. 2000, p.~201)
...
\emph{Alisina}: Following Williams \emph{et al}. (2000) table 15 (their
character 54).
...
Williams \emph{et al}. (2000), fig. 81.

\hypertarget{Botsfordia-coding-85}{}
\emph{Botsfordia}: Following Williams \emph{et al}. 1998T, appendix 2.

\hypertarget{Clupeafumosus_socialis-coding-85}{}
\emph{Clupeafumosus socialis}: Following reconstruction of
\emph{Hadrotreta} by Williams (2000), fig. 51, which exhibits distinct
...
\emph{Gasconsia}: Following the coding of Williams \emph{et al}. (2000),
table 15.

\hypertarget{Glyptoria-coding-85}{}
\emph{Glyptoria}: Scored as ``dispersed'' by Williams \emph{et al}.
(1998T) \ldots{} but then so is \emph{Kutorgina}, which Bassett \emph{et
al} (2001) score as radial.

Williams \emph{et al}. (2000) state, for superfamily Protorthida,
``dorsal adductor scars probably linear'', which fits in the category of
...
\emph{Haplophrentis carinatus}: Moysiuk \emph{et al}. (2017) reconstruct
distinct left and right attachment scars, consistent with general
situation in hyoliths (see Dzik 1980); it is unclear whether additional
...
\emph{et al}. 2007); coded as ``dispersed'' by Williams \emph{et al}.
(2000) in table 15.
...
\emph{Micromitra}: Williams \emph{et al}. (1998T) code as ``dispersed'',
but have a less divided scheme of character states and disagree with
other sources in some codings (e.g.~Bassett \emph{et al}. 2001, in
Kutorginates). Williams \emph{et al}. (2000) do not describe
...
After Bassett \emph{et al}. (2001) character 11.
...
\emph{Botsfordia}: Following description of Popov (1992).

\hypertarget{Coolinia_pecten-coding-86}{}
\emph{Coolinia pecten}: Not reported by Williams \emph{et al}. (2000),
nor Bassett \& Popov (2017), nor explicitly by Dewing (2001).
...
in criss-crossing pairs'' -- Balthasar 2009T.
...
equivalent to \emph{Lingula} by Williams \emph{et al} 2000, so
...
Based on character 11 in Zhang \emph{et al}. (2014).\\
...
the possible exception of the paterinates (Williams \emph{et al}., 2000,
p.~32).
...
al}. (2000, p.~32) that these muscle are absent in all carbonate-
shelled brachiopods.

\hypertarget{Askepasma_toddense-coding-87}{}
\emph{Askepasma toddense}: According to the statement of Williams
\emph{et al}. (2000, p.~32) that the presence of these muscles in
paterinates is uncertain.

\hypertarget{Botsfordia-coding-87}{}
\emph{Botsfordia}: Implicitly taken as present in Popov (1992), though
not marked in diagrams -- suggesting not strongly developed.

\hypertarget{Clupeafumosus_socialis-coding-87}{}
\emph{Clupeafumosus socialis}: This character is coded based on the
score of Acrotreta in Zhang \emph{et al}. (2014), and statement in
Williams \emph{et al}. (2000, P.32).

\hypertarget{Coolinia_pecten-coding-87}{}
\emph{Coolinia pecten}: According to the statement of Williams \emph{et
al}. (2000, p.~32) that these muscle are absent in all carbonate-shelled
brachiopods.

\hypertarget{Eoobolus-coding-87}{}
\emph{Eoobolus}: Not remarked upon by Balthasar (2009T).

\hypertarget{Kutorgina_chengjiangensis-coding-87}{}
\emph{Kutorgina chengjiangensis}: According to the statement of Williams
\emph{et al}. (2000, p.~32) that these muscle are absent in all
carbonate- shelled brachiopods, and the coding for kutorginids in Zhang
\emph{et al}. (2014).

\hypertarget{Micromitra-coding-87}{}
\emph{Micromitra}: Williams \emph{et al}. (2000, p.~32) are uncertain
about the presence of these muscles in the paterinates. Zhang \emph{et
al}. (2014) code absence in Paterinida, but without specifying evidence;
we follow their coding here.

\hypertarget{Mummpikia_nuda-coding-87}{}
\emph{Mummpikia nuda}, \emph{Tomteluva perturbata}: Though Williams
\emph{et al}. (2000, p.~32) state that these muscles are absent in all
carbonate-shelled brachiopods, their existence cannot be discounted with
certainty in this taxon, which is therefore coded not presently
available.

\hypertarget{Novocrania-coding-87}{}
\emph{Novocrania}: Following Zhang \emph{et al}. (2014), and the
statement of Williams \emph{et al}. (2000) that such muscles are absent
in all calcite-shelled brachiopods.
...
\emph{Terebratulina}: Williams \emph{et al}. (2000, p.~32) state that
these muscles are absent in all carbonate-shelled brachiopods.

\subsection*{{[}88{]} Muscle scars: Unpaired median (levator
...
certain calcitic brachiopods (Williams \emph{et al}. 2000; see fig. 89,
character 34 in table 13).

\hypertarget{Alisina-coding-88}{}
\emph{Alisina}, \emph{Kutorgina chengjiangensis}, \emph{Nisusia
sulcata}: Following table 13 in Williams \emph{et al}. 2000.

\hypertarget{Coolinia_pecten-coding-88}{}
\emph{Coolinia pecten}: Not reported in Dewing (2001).

\hypertarget{Craniops-coding-88}{}
\emph{Craniops}: See fig. 90 in Williams \emph{et al}. 2000.

\hypertarget{Gasconsia-coding-88}{}
\emph{Gasconsia}: Williams \emph{et al}. 2000 code an unpaired medial
...
trimerellid. Hanken and Harper (1985, p.~249 and text-fig. 2) explicitly
...
\emph{Novocrania}: Following table 13 in Williams \emph{et al}. 2000
(for \emph{Novocrania}).
...
After Bassett \emph{et al}. (2001) character 9.
...
After Bassett \emph{et al}. (2001) character 10.
...
See Fig. 284 in Williams \emph{et al} (1997).\\
...
ventral (but not dorsal) valves of brachiopods (Williams \emph{et al}
1997, fig. 322), and which occur in the morph A shell of
...
termed a pseudointerarea (paraphrasing Williams \emph{et al}. 1997).
...
Siphonotretidae coding in Williams \emph{et al}. (2000), table 6.
...
\emph{Longtancunella chengjiangensis}: Zhang \emph{et al}. (2011T) note
that ``all evidence of a pseudointerarea is lacking'', but the
...
\emph{Micrina}: = Sellate sclerite duplicature (Holmer \emph{et al}.
2008).

\hypertarget{Micromitra-coding-92}{}
\emph{Micromitra}: ``Dorsal pseudointerarea usually well defined, low,
anacline to catacline'' -- Williams \emph{et al}. 2000.
...
taphonomic artefact is difficult to assess'' -- Balthasar 2008, p.~276.

\hypertarget{Nisusia_sulcata-coding-92}{}
\emph{Nisusia sulcata}: Cardinal area (interarea) present -- with
reference to Holmer \emph{et al}. (2018E).
...
fossil material or its reconstruction (Zhang \emph{et al}. 2014), but
the two-dimensional preservation style of Chengjiang material makes
details of dorsal valve difficult to distinguish, and the possibility of
a diminutive pseudointerarea cannot be excluded with confidence.
...
\emph{Eoobolus}: Essentially planar; see Balthasar (2009T), fig. 4a.
...
Following character 29 in Williams \emph{et al}. (2000), table 9 (which
...
preserved, but appears to have a median groove (Holmer \& Caron, 2006).
...
priscus} is undivided (Popov \emph{et al}. 2009), but in other species
it is divided by a ``wide, poorly defined median groove'' (Williams
\emph{et al}. 2000). Coded, therefore, as polymorphic.
...
opening at the hinge line in fossil material in Zhang \emph{et al}.
2007A or Zhang \emph{et al}. 2011T.
...
A simplification of character 5 in Bassett \emph{et al}. 2001.
...
After Bassett \emph{et al}. (2001) character 12.\\
...
\emph{Alisina}, \emph{Glyptoria}: Bassett \emph{et al}. (2001) score as
...
floor of notothyrial cavity'': Williams \emph{et al}. 2000, regarding
Kutorginata.\\
Bassett \emph{et al}. (2001) score as absent in Table 18.1.
...
\emph{Nisusia sulcata}: Bassett \emph{et al}. (2001) score as absent in
Table 18.1.\\
``Dorsal diductor scars impressed on floor of notothyrial cavity'':
Williams \emph{et al}. 2000, regarding Kutorginata.
...
\emph{Acanthotretella spinosa}: Not described by Holmer \& Caron (2006),
but an unannotated linear feature corresponds to the position of a
median septum. Without detailed study of the specimen, we opt to score
this as ambiguous.

\hypertarget{Antigonambonites_planus-coding-99}{}
\emph{Antigonambonites planus}: Weakly developed septum evident in
internal cast: Williams \emph{et al}. 2000, fig. 508.2e.

\hypertarget{Botsfordia-coding-99}{}
\emph{Botsfordia}: ``dorsal interior with narrow anterior projection
extending to midvalve, bisected by median ridge'' -- Williams \emph{et
al}. 2000.

\hypertarget{Clupeafumosus_socialis-coding-99}{}
\emph{Clupeafumosus socialis}: Prominent process evident (Topper
\emph{et al}., 2013R).
...
al}. (2007); we consider their `ventral' valve to be the dorsal valve.

The structure is unambiguously figured (e.g.~fig. 5.1 in Chen \emph{et
al}. 2007), contra its coding as absent in Williams \emph{et al}. 2000
and its lack of mention in Williams \emph{et al}. 2007 or Zhang \emph{et
al}. 2009.
...
in Aboriginella (also Lingulellotretidae) (Williams \emph{et al}. 2000,
fig. 34).

\hypertarget{Lingulosacculus-coding-99}{}
\emph{Lingulosacculus}: It is not possible to determine, based on the
material presented in Balthasar \& Butterfield (2009E), whether the
anterior projection of the visceral area in the dorsal valve corresponds
to a medial septum in the underlying shell.
...
\emph{Nisusia sulcata}: Fig. 125 in Williams \emph{et al}. (2000).

\hypertarget{Novocrania-coding-99}{}
\emph{Novocrania}: Median process evident: Williams \emph{et al}. (2000)
fig. 100.2a, d.
...
by a short median ridge.'' -- Popov \emph{et al}. 2009.

\hypertarget{Ussunia-coding-99}{}
\emph{Ussunia}: Following char 42 in table 15 in Williams \emph{et al}.
2000.
...
After Bassett \emph{et al}. (2001) character 13. See
...
See Fig. 284 in Williams \emph{et al}. (1997) for depiction of terms.
...
\emph{Heliomedusa orienta}: Williams \emph{et al}. (2000, 2007)
reconstruct mixoperipheral growth in the ventral valve (though Chen
\emph{et al}. (2007) reconstruct the valves the other way round, i.e.~it
is the ventral valve that grows holoperipherally, and the dorsal
mixoperipherally).
...
lateral plate -- see Larsson \emph{et al}. 2014, fig. 2a, c.

\hypertarget{Siphonobolus_priscus-coding-105}{}
\emph{Siphonobolus priscus}: Initially holoperipheral (Popov \emph{et
al}. 2009, p.~159), then on the brink of being mixoperipheral in
adulthood, so coded as polymorphic.
...
\emph{Nisusia sulcata}: Ventral valve larger (see Williams \emph{et al}.
2000, fig. 126.).

\hypertarget{Siphonobolus_priscus-coding-106}{}
\emph{Siphonobolus priscus}: Ventribiconvex (Popov \emph{et al}. 2009).
...
\emph{Ussunia}: Subequally biconvex (Williams \emph{et al}. 2000,
p.~192).
...
area'' by Williams \emph{et al}. (2000, p.162), who code both an
interarea and a pseudointerarea as absent in trimerellids.

\hypertarget{Heliomedusa_orienta-coding-107}{}
\emph{Heliomedusa orienta}: Zhang \emph{et al}. (2009) report a moderate
to somewhat developed ventral pseudointerarea, confirmed by Williams
\emph{et al} (2007).
...
pseudointerarea is lacking'' -- Zhang \emph{et al}. 2011T -- the region
of the shell between the strophic hinge line and the colleplax (fig. 2
in Zhang \emph{et al}. 2011T) is distinct from the rest of the shell;
...
\emph{Mickwitzia muralensis}: Termed an interarea by Balthasar (2004).
...
poorly defined'' -- Williams \emph{et al}. 2000.
...
\emph{Ussunia}: Following char 17 in table 15 of Williams \emph{et al}.
2000.
...
\emph{Botsfordia}: Inward-growing; see Skovsted \& Holmer (2005), pl. 4.

\hypertarget{Clupeafumosus_socialis-coding-108}{}
\emph{Clupeafumosus socialis}: See Topper \emph{et al}. (2013R).

\hypertarget{Eoobolus-coding-108}{}
\emph{Eoobolus}: See for example Skovsted \& Holmer (2005), pl. 3.
...
\emph{Botsfordia}: See Skovsted \& Holmer (2005), pl. 3, fig. 14.
...
\emph{Micromitra}: Essentially planar; see fig. 6 in Ushatinskaya 2016P.
...
Benedetto (2009), this taxon (see Williams \emph{et al}. 2000, fig. 508)
does not express the deeply conical ventral valve that this character is
intended to reflect. It is charitably coded as ambiguous.

\hypertarget{Clupeafumosus_socialis-coding-111}{}
\emph{Clupeafumosus socialis}: Entire valve length -- see schematic in
Williams \emph{et al}. (1997), fig. 286.

\hypertarget{Coolinia_pecten-coding-111}{}
\emph{Coolinia pecten}: See fig. 485 in Williams \emph{et al}. 2000.

\hypertarget{Gasconsia-coding-111}{}
\emph{Gasconsia}: ``ventral cardinal interarea low, apsacline, with
narrow, poorly defined homeodeltidium'' -- Williams \emph{et al}. 2000,
p.~186.

\hypertarget{Kutorgina_chengjiangensis-coding-111}{}
\emph{Kutorgina chengjiangensis}: This taxon (see Williams \emph{et al}.
2000, fig. 129; Popov 1992, fig. 1) comes close to expressing the deeply
conical ventral valve that this character is intended to reflect, though
this is not always so pronounced (e.g.~Williams \emph{et al}. 2000, fig.
125). It is therefore coded as ambiguous.

\hypertarget{Mickwitzia_muralensis-coding-111}{}
\emph{Mickwitzia muralensis}: Often not prominently high (Skovsted \&
Holmer, 2003; Balthasar, 2004), though in some cases (e.g.~Butler
\emph{et al}. 2015) the ventral valve approaches the conical shape that
this character is intended to capture. Coded as polymorphic.

\hypertarget{Nisusia_sulcata-coding-111}{}
\emph{Nisusia sulcata}: Scored as high in data matrix of Benedetto
(2009), and depicted as such in Williams \emph{et al}. (2000, fig. 125)
and Popov (1992, fig. 1); but not high in all specimens (e.g.~Williams
\emph{et al}. 2000, fig. 126). It is therefore coded as polymorphic.

\hypertarget{Novocrania-coding-111}{}
\emph{Novocrania}: Low cone.

\hypertarget{Orthis-coding-111}{}
\emph{Orthis}: Scored `Low' for \emph{Eoorthis} by Benedetto (2009);
assumed same in \emph{Orthis}.

\hypertarget{Salanygolina-coding-111}{}
\emph{Salanygolina}: Whereas Williams \emph{et al}. (2000, p.~156)
describe the ventral pseudointerarea as high, the shell lacks the deeply
conical aspect that this character is intended to capture; we thus code
the taxon as ambiguous.
...
proposed by Popov (1992), and seemingly corroborated by observations of
Ushatinskaya \& Korovnikov (2016R), who note that the propareas of the
\emph{Botsfordia} ventral valve sometimes merge to form an elongate
...
\emph{Askepasma toddense}: Homeodeltidium absent (Williams \emph{et al}.
2000, p.~153); deltidium is open (see Topper \emph{et al}. 2013T, fig.
4).
...
\emph{Clupeafumosus socialis}: Following Popov (1992), the larval
delthyrium is sealed in adults by outgrowths of the posterolateral
margins of the shell.

\hypertarget{Eoobolus-coding-112}{}
\emph{Eoobolus}: See for example fig. 5 in Balthasar 2009T.
...
2c is not discussed in the text of Zhang \emph{et al}. 2011T, so the
...
\emph{Micrina}: Opening inferred by Holmer \emph{et al}. (2008).
...
acrotretids (Popov 1992), and hence corresponding to a basally sealed
delthyrium.
...
Following Popov (1992), the larval delthyrium of acrotretids and allied
taxa is understood to be sealed in adults by outgrowths of the
posterolateral margins of the shell. The resultant round or
teardrop-shaped foramen corresponds the delthyrium.

\hypertarget{Askepasma_toddense-coding-113}{}
\emph{Askepasma toddense}: Prominently triangular (see Topper \emph{et
al}. 2013T, fig. 2).

\hypertarget{Clupeafumosus_socialis-coding-113}{}
\emph{Clupeafumosus socialis}: Following the model of Popov (1992).
...
maturity (fig. 10 in Balthasar 2004). This opening is conceivably
homologous with the pedicle foramen of acrotretid brachiopods and their
ilk. To reflect this possible homology, \emph{Mickwitzia} is coded as
polymorphic (triangular/round).
...
Chen \emph{et al}. (2007) propose that an oval to rhombic foramen
characterises the discinids (and \emph{Heliomedusa}, though the foramen
in this taxon has since been reinterpreted by Zhang \emph{et al}. (2009)
as an impression of internal tissue).

\hypertarget{Lingulellotreta_malongensis-coding-114}{}
\emph{Lingulellotreta malongensis}: Oval (Williams \emph{et al}. 2000).

\hypertarget{Mickwitzia_muralensis-coding-114}{}
\emph{Mickwitzia muralensis}: Wider than long: see fig. 10 in Balthasar
2004.
...
Inapplicable in taxa with a round delthiruym (generated by overgrowth of
the delthyrial opening by posterolateral parts of the shell, per Popov
1992).

\hypertarget{Askepasma_toddense-coding-115}{}
\emph{Askepasma toddense}: Open (Topper \emph{et al}. 2013T).

\hypertarget{Botsfordia-coding-115}{}
\emph{Botsfordia}: See pl. 3 fig. 15 in Skovsted \& Holmer (2005).
...
\emph{Glyptoria}: Coded as open by Williams \emph{et al}. (1998T).

\hypertarget{Micromitra-coding-115}{}
\emph{Micromitra}: Williams \emph{et al}. 2000, fig. 83.3.

\hypertarget{Nisusia_sulcata-coding-115}{}
\emph{Nisusia sulcata}: ``Covered only apically by a small convex
pseudodeltitium'' -- Holmer \emph{et al}. 2018E.
...
\emph{Micromitra}: Completely covered (Williams \emph{et al}. 2000, fig.
83.3).
...
homeodeltidium of \emph{Micromitra} (Larsson \emph{et al} 2014).
...
delthyrium, covered by a convex pseudodeltidium'' -- Holmer \emph{et
al}. 2008.
...
with \emph{Coolinia} and other Chileata (Holmer \emph{et al}. 2009,
p.~6).
...
\emph{Antigonambonites planus}: Convex (Williams \emph{et al}. 2000,
fig. 508).
...
material presented in Zhang \emph{et al} (2007R), or indeed for other
species in the genus (e.g.~Williams \emph{et al}. 2000; Skovsted \&
Holmer 2005; Holmer \emph{et al}. 2018T).

\hypertarget{Mickwitzia_muralensis-coding-118}{}
\emph{Mickwitzia muralensis}: Convex (see Balthasar 2004, fig. 4B).

\hypertarget{Micrina-coding-118}{}
\emph{Micrina}: Convex deltoid (Holmer \emph{et al}. 2008T).

\hypertarget{Micromitra-coding-118}{}
\emph{Micromitra}: Gently convex (see Williams \emph{et al}. 2000, fig.
83.3).

\hypertarget{Nisusia_sulcata-coding-118}{}
\emph{Nisusia sulcata}: Convex in \emph{Nisusia} (see Rowell and Caruso,
1985, fig. 8.4).

\hypertarget{Paterimitra-coding-118}{}
\emph{Paterimitra}: Gently convex (see Williams \emph{et al}. 2000, fig.
83.1).
...
Cambrian Chileida'' -- Holmer \emph{et al}. 2009, p.~9.

\hypertarget{Tomteluva_perturbata-coding-118}{}
\emph{Tomteluva perturbata}: Convex (Streng \emph{et al}. 2016).
...
After Bassett \emph{et al}. (2001) character 18, ``Hinge furrows on
lateral sides of pseudodeltidium''.
...
\emph{Glyptoria}: Coded as absent in Bassett \emph{et al}. 2001 (table
18.1).

\hypertarget{Kutorgina_chengjiangensis-coding-119}{}
\emph{Kutorgina chengjiangensis}, \emph{Nisusia sulcata}: Coded as
present in Bassett \emph{et al}. 2001 (table 18.1).
...
of the brephic shell, at) the umbo (see Popov 1992, pp.~407--411 and
fig. 3 for discussion). In some cases, an internal pedicle tube attests
to this origin -- potentially corresponding to the pedicle groove of
lingulids. As such, the pedicle foramen of acrotretids and allies is not
originally situated at the umbo; it is instead understood to represent a
...
Topper \emph{et al}. (2013R) is at the ventral valve umbo. No evidence
of an internal pedicle tube is present, but we follow Popov (1992) in
inferring the encapsulation of the pedicle foramen.
...
umbonal perforations (Skovsted \emph{et al} 2015), but these are not
...
conceivably equivalent to the setal tubes of \emph{Micrina} (Holmer
\emph{et al}. 2011). The A1 sclerite of D. bacata has a structure that
is arguably similar to the `colleplax' of \emph{Paterimitra}. But the
homology of any of these structures to the umbonal aperture of
brachiopods is difficult to establish.
...
illustrated by Chen \emph{et al}. (2007: Figs. 4, 6, 7) in fact is the
mold of a three-dimensionally preserved visceral cavity.'' -- Zhang
\emph{et al}. 2009.
...
\emph{Mickwitzia muralensis}: The umbo itself is imperforate (Balthasar
2004).
...
preservation (Holmer \& Caron 2006).

\hypertarget{Alisina-coding-121}{}
\emph{Alisina}: Seemingly circular (Zhang \emph{et al}. 2011A).
...
nearly circular in others; see Topper \emph{et al}. (2013R).
...
discinid affinity (Chen \emph{et al}. 2007).
...
\emph{Nisusia sulcata}: ``close to circular'' (Holmer \emph{et al}.
2018E).

\hypertarget{Salanygolina-coding-121}{}
\emph{Salanygolina}: Essentially circular (Holmer \emph{et al}. 2009,
fig. 4).
...
cicatrix or pedicle sheath; Bassett \emph{et al}. (2008) consider these
...
\emph{Craniops} bears a cicatrix (Williams \emph{et al}. 2000). Also
coded as present in their table 15.
...
as with the `pedicle foramen' of Chen \emph{et al}. (2007), this
structure represents internal organs rather than a cicatrix proper
(Zhang \emph{et al}. 2009); as such it has been recorded as ambiguous.

\hypertarget{Kutorgina_chengjiangensis-coding-122}{}
\emph{Kutorgina chengjiangensis}: The umbonal region of kutorginides
``clearly lacks a pedicle sheath'' (Holmer \emph{et al}. 2018T).
...
the pedicle is interpreted as a colleplax (Zhang \emph{et al}. 2011T),
though the authors make no comparison with the pedicle capsule exhibited
by extant terebratulids (see Holmer \emph{et al}. 2018E).

\hypertarget{Micrina-coding-122}{}
\emph{Micrina}: Absent in \emph{Micrina} (Holmer \emph{et al}. 2011).
...
and foramen with attachment pad'' in \emph{Salanygolina} (Popov \emph{et
al}. 2009).
...
(Streng \emph{et al}. 2016). It could conceivably correspond to an
...
Chen \emph{et al}. (2007) observe a median septum in what they interpret
as the ventral valve of \emph{Heliomedusa}, and the ventral valve of
\emph{Discinisca}, which they propose points to a close relationship.
...
in the ventral valve (Topper \emph{et al}. 2013R).

\hypertarget{Eoobolus-coding-123}{}
\emph{Eoobolus}: Prominent median septum (fig. 4d, e in Balthasar
2009T).
...
\emph{Discinisca} by Chen \emph{et al}. 2007; assumed present also in
\emph{Pelagodiscus}.

\hypertarget{Siphonobolus_priscus-coding-123}{}
\emph{Siphonobolus priscus}: Present; see Popov \emph{et al}. 2009, fig.
5J.
...
After character 11 in Williams \emph{et al}. (1998T). Coded as
transformational as it is possible that maintaining a smooth shell
without occasional prominent ridges requires greater secretory control.

\hypertarget{Askepasma_toddense-coding-124}{}
\emph{Askepasma toddense}, \emph{Micromitra}, \emph{Glyptoria},
\emph{Kutorgina chengjiangensis}, \emph{Salanygolina}: Following
appendix 2 in Williams \emph{et al}. (1998T).
...
\emph{Botsfordia}: Following Williams \emph{et al}. 1998T, appendix 2.\\
Pustules are arranged along concentric growth lines (Skovsted \& Holmer,
2005), so are not treated as a distinct ornamentation.
...
\emph{Eccentrotheca} sclerites (Skovsted \emph{et al}. 2011).
...
ridges adorn both valves (Moysiuk \emph{et al}. 2017); these are more
...
as concentric fila (Chen \emph{et al}., 2007, P.43), and also scored as
it in Williams \emph{et al}. (2000, pp.160--163).
...
\emph{Novocrania}: Irregular ridges externally (Williams \emph{et al}.
2000).
...
\emph{Pelagodiscus atlanticus}: Only growth lines evident (Williams
\emph{et al}. 2000).

\hypertarget{Terebratulina-coding-124}{}
\emph{Terebratulina}: Single ridge evident in Williams \emph{et al}.
(2006) fig. 1425.1a interpreted as interruption ot growth rather than
inherent feature, so coded as absent (i.e.~smooth).
...
After character 11 in Williams \emph{et al}. (1998T).

\hypertarget{Alisina-coding-125}{}
\emph{Alisina}: Seemingly asymmetric (Williams \emph{et al}. 2000, fig.
122.3c; Zhang \emph{et al}. 2011A, Fig. 1).
...
\emph{Dailyatia}: Clear asymmetry (Skovsted \emph{et al}. 2015).

\hypertarget{Eccentrotheca-coding-125}{}
\emph{Eccentrotheca}: Ornament, such as it is, is asymmetric, with
prominent outer faces (Skovsted \emph{et al}. 2011).
...
either (Holmer \emph{et al}. 2008). Coded as ambiguous.

\hypertarget{Novocrania-coding-125}{}
\emph{Novocrania}: Clear outer faces (Williams \emph{et al}. 2000, fig.
100.2b).
...
polygonal pits'' -- Williams \emph{et al}. 2000, p153; figs on p.155.
...
\emph{et al}. (2000, p167).

\hypertarget{Glyptoria-coding-126}{}
\emph{Glyptoria}: ``Coarsely costate'' -- Williams \emph{et al}. (2000,
p710).
...
radial rows of tubercles'' -- Popov \emph{et al}. 2009.
...
(2007) are pyritized spinelike setae -- see pp.~2580--2590 in Williams
\emph{et al}. (2007).

\hypertarget{Nisusia_sulcata-coding-127}{}
\emph{Nisusia sulcata}: Bears numerous small, hollow spines (Williams
\emph{et al}. 2000).
...
\emph{Acanthotretella spinosa}: Holmer \& Caron (2006) note the absence
of brittle breakage, interpreted as indicating the absence of a material
mineralized component to the shells. The preservation is strikingly
different from that of other Burgess Shale brachiopods, ruling out a
primarily calcitic or phosphatic composition. The two-dimensional nature
of the preservation also differs from that of co-occurring aragonitic
taxa (hyoliths; Holmer \& Caron 2006 p.~273), indicating that any
mineralization was minor at best.

Holmer \& Caron (2006, p.~286) suggest that it is more likely that a
(minor) mineral component was present than that it was not, though
without providing an uncontestable rationale. To be as conservative as
possible, we therefore code this taxon as ambiguous.
...
\emph{Gasconsia}: Confirmed in Trimerella by Balthasar \emph{et al}.
2011.

\hypertarget{Heliomedusa_orienta-coding-128}{}
\emph{Heliomedusa orienta}: ``Shell originally organophosphatic, but may
generally have been poorly mineralized'' -- Williams \emph{et al}. 2007
...
\emph{et al}. (2014), but with no explanation.\\
Cracks within shells of Chengjiang specimens (e.g.~Zhang \emph{et al}.
2007N, fig. 3) demonstrate that the shells were originally mineralized,
but not the identity of the original biomineral. This said, phosphatized
material from Kazakhstan (Holmer \emph{et al}. 1997) is attributed to
the same species; presuming this phosphate to be original and the
material to be conspecific, \emph{L. malongensis} is coded as having
phosphatic shells.
...
constitution was thus either organic or aragonitic (Balthasar \&
Butterfield 2009E).
...
probably entirely soft and organic'' -- Zhang \emph{et al}. 2011T.
...
calcitic shells of \emph{M. nuda}'' (Balthasar 2008).

\hypertarget{Novocrania-coding-128}{}
\emph{Novocrania}: Ventral valve uncalcified in extant forms or
sometimes thin (Williams \emph{et al}., 2000), but coded as calcitic as
...
mineralised and anticipated to be phosphatic (Holmer \emph{et al}.
2009).
...
Williams \emph{et al}. (1996) identify glycoprotein-based organic
scaffolds as distinct from those comprising glycosaminoglycans (GAGs),
chitin and collagen. This character can only be scored for extant taxa.
...
discinids by Williams \emph{et al} (1996).
...
rhynchonelliform shells (Fig. 1891, 1896)'' -- Williams \emph{et al}.
2007, p.~2830.

\hypertarget{Siphonobolus_priscus-coding-129}{}
\emph{Siphonobolus priscus}: Lenticular chambers in siphonotretid shells
interpreted as degraded GAG residue (Williams \emph{et al}. 2004).
...
Following character 9 in Williams \emph{et al}. (1998T); see their
p228--230 for a discussion of how this might be inferred from fossil
material.
...
\emph{Botsfordia}, \emph{Eoobolus}: Coded as flexible in Williams
\emph{et al}. 1998T, Appendix 2.

\hypertarget{Pelagodiscus_atlanticus-coding-131}{}
\emph{Pelagodiscus atlanticus}: Flexible (Williams \emph{et al}. 1998T).
...
succession'' -- Williams \emph{et al}. 2007.
...
\emph{Mummpikia nuda}: Balthasar (2008) considers the single
mineralogical layer, which comprises phosphatic rods and laminae, to
...
layers'' -- Williams \emph{et al}. 2004.
...
(Larsson \emph{et al}. 2014)

Williams \emph{et al}. (2000) identify cross-bladed laminae as
...
secondary layer (Williams \emph{et al}. 2000, p.~724).
...
preserved in canal walls (Williams \emph{et al}. 2004).
...
\emph{Siphonobolus priscus}: (Williams \emph{et al}. 2004).
...
\emph{Botsfordia}: Skovsted \& Holmer 2003.
...
\emph{Mummpikia nuda}: It is conceivable that the rods (Balthasar 2008)
correspond to the polygonal ornament observed in other taxa; coded as
ambiguous.
...
layer. See Williams 1997, p303ff, and Balthasar 2008, p273, for
discussion.

Tubules described in hyoliths by Kouchinsky (2000) measure around 10 μm
in diameter, making them an order of magnitude wider than lingulid
canals.

This said, Balthasar (2008) considers the rod-like tubules within the
columnar shell microstructure of \emph{Mickwitzia} cf.~occidens (1--3 μm
wide, Skovsted \& Holmer 2003), acrotretides (1 μm wide, see Holmer
1989, Zhang \emph{et al}. 2016) and lingulellotretids (100 nm wide,
Cusack \emph{et al} 1999) as equivalent to lingulid canals.

\emph{Micrina} exhibits both punctae and canals (Harper \emph{et al}.
2017), challenging Carlson's contention (in Williams \emph{et al}. 2007)
that the structures are potentially homologous as shell perforations.
...
columns (e.g.~Zhang \emph{et al}. 2016).
...
Williams \emph{et al}. (2007); Skovsted \& Holmer (2003).

\hypertarget{Micrina-coding-137}{}
\emph{Micrina}: Acrotretid laminae bear characteristic columns
(e.g.~Zhang \emph{et al}. 2016); a similar fabric has been reported, and
assumed homologous, in \emph{Micrina} (Butler \emph{et al}. 2012).
...
\emph{Namacalathus} (Zhuravlev \emph{et al}. 2015), and interpreted as
evidence for a Lophophorate affinity. Though the structures are not
necessarily directly equivalent, the hypothesis of homology is followed
here.
...
diameter of c. 20 μm (Williams \emph{et al}. 2004, text-fig. 2a),
...
Punctae are 10--20 μm wide and represent multicellular extensions of the
...
compares with the much smaller canals (\textless{} 1 μm in diameter) of
\emph{M. nuda}. The only type of vertical structure that fits the size
and nature of the canals of the Mural obolellids are the canals of
linguliform brachiopods, which range in width from 180 to 740 nm and are
occupied by proteinaceous strands in extant taxa (Williams \emph{et al}.
1992; Williams \emph{et al}. 1994; Williams \emph{et al}. 1997). In
contrast to obolellid canals, however, linguliform canals are not known
to penetrate the entire shell but terminate in organic-rich layers
(Williams 1997). Based on these considerations it would, therefore, be
misleading to call obolellid shells punctate (they are as
much''punctate" as acrotretids or other linguliforms); rather their
shell structure should be called canaliculate (Williams 1997)."
...
bundles of hyolith shells (Kouchinsky 2000) are c. 10 μm wide, making
them an order of magnitude larger than the canals that characterize
lingulid valves, and a similar scale to punctae. This said, they have
only been reported in a putative allathecid, so the presence of
equivalent structures in hyolithids has never been demonstrated.

\hypertarget{Heliomedusa_orienta-coding-138}{}
\emph{Heliomedusa orienta}: `Identical' to those in \emph{Mickwitzia} --
see Williams \emph{et al}. 2007.
...
chambers contained setae; following Carlson in Williams \emph{et al}.
2007, the punctae may or may not be homologous as punctae, but are
likely homologous as shell perforations; both these perforations and
those of \emph{Micrina} were associated with setae, even if their
equivalence bay be with juvenile vs adult setal structures in modern
brachiopods (Balthasar 2004, p.~397).
...
size (Williams 1997). Punctae are 10--20 μm wide and represent
...
linguliform brachiopods, which range in width from 180 to 740 nm and are
occupied by proteinaceous strands in extant taxa (Williams \emph{et al}.
1992, 1994; Williams 1997).'' -- Balthasar 2008.
...
sulcata}: Scored absent in data matrix of Benedetto (2009).
...
acrotretids by Zhang \emph{et al}. (2016) is on the internal surface of
the shell.
...
\emph{Amathia}: ``Mature eggs commonly measure about 200 μm in
diameter'' \citep{Franzen1977}; the larva is a similar size
\citep{Reed1982}.

\hypertarget{Dentalium-coding-147}{}
\emph{Dentalium}: Egg size can vary from 60--200 μm in scaphopods, but
in \emph{Dentalium} the eggs are large \citep{DufresneDube1983}.

\hypertarget{Flustra-coding-147}{}
\emph{Flustra}: ``Mature eggs commonly measure about 200 μm in
...
small egg size (Ruppert \emph{et al}. 2004). Carlson (1995) codes
phoronids as polymorphic, as some members of the phylum have eggs of
each size.

\hypertarget{Serpula-coding-147}{}
\emph{Serpula}: c. 50 μm in \emph{Hydroides} \citep{Miles2007}.
...
egg size'' -- Popov \emph{et al}. 2009.

\hypertarget{Sipunculus-coding-147}{}
\emph{Sipunculus}: c. 200 μm in diameter \citep{Rice1988}.
...
\emph{Novocrania}, \emph{Pelagodiscus atlanticus}, \emph{Lingula},
\emph{Terebratulina}: Following Hodgson \& Reunov (1994).
...
\emph{tenuis} (Hodgson \& Reunov, 1994).
...
\emph{Lingula}: Pear-shaped (Fukumoto 2003).
...
\emph{Novocrania}: Needle-shaped (Afzelius \& Ferraguti, 1978).

\hypertarget{Pelagodiscus_atlanticus-coding-152}{}
\emph{Pelagodiscus atlanticus}: Pear-shaped (Hodgson \& Reunov, 1994).

\hypertarget{Phoronis-coding-152}{}
\emph{Phoronis}: Needle-shaped (Reunov \& Klepal, 2004).
...
\emph{Terebratulina}: Disc-shaped (in \emph{Kraussina}) (Hodgson \&
Reunov, 1994).
...
Hodgson \& Reunov 1994 describe the \emph{Discinisca} acrosome as having
``an electron-lucent centre and an electron-dense outer region'', and
state that this trait is characteristic of inarticulate brachiopods.
...
\emph{Lingula}: Clear differentiation of marginal area (Fukumoto 2003).
...
secondarily lost (Reunov \& Klepal 2004).
...
\emph{Novocrania}: Four mitochondria (Afzelius \& Ferraguti, 1978).
...
spermiogenesis to become a pair of mitochondria (Reunov \& Klepal 2004).
...
\emph{Novocrania}: Two orthogonal centrioles (Afzelius \& Ferraguti
1978).
...
\emph{Phoronis}: Only one centriole in spermatzoon (Reunov \& Klepal
2004, p.~7), but centrioles are perpendicularly oriented in
spermatogonia (ibid., p.~2).
...
of \citet{Brazeau2018} as ``inapplicable-corrected'' parsimony, as it
corrects for errors arising from the handling of inapplicable data in
standard Fitch parsimony.
...
relationship is preferred by Fitch and inapplicable-corrected parsimony
analyses.
...
been tricky to resolve \citep{Holmer2018Theattachment}.
Inapplicable-corrected parsimony places them sister to
Rhynchonelliforms; Fitch parsimony places them as sister to the
Chileids; and Bayesian analysis conservatively fails to distinguish
between these two possibilities. These results are broadly in accord
with previous proposals \citep{Holmer2018Evolutionarysignificance}. The
protorthid \emph{Glyptoria} is the earliest diverging of the included
...
Inapplicable-corrected parsimony, in contrast, supports the
interpretation of \emph{Yuganotheca} as a stem-group brachiopod
\citep{Zhang2014Anearly}.
\item[Paterinids]
...
affiliation, but Bayesian and inapplicable-corrected parsimony analyses
place it as a stem-group mollusc, whereas Fitch parsimony includes it in
the molluscan crown. \emph{Halkieria}, \emph{Cotyledion},
\emph{Eccentrotheca} and \emph{Dailyatia} are recovered as close
relatives, forming a grade in the brachiopod stem group under
inapplicable-corrected parsimony, a clade in the molluscan stem group in
Fitch parsimony, and a polytomy at a deeper node under Bayesian
analysis.\\
\emph{Namacalathus} is reconstructed as a basal Brachiozoan under
inapplicable-corrected parsimony and Bayesian analysis, and a stem-group
brachiopod under Fitch parsimony.
\end{description}
