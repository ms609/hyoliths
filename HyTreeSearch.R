...
for (i in 1:100) {
  cat("\n\n############################# I =", i)
  plot(iw.tree <- IWRatchet(iw.tree, my_data, verbosity=2, concavity=k,
                       swappers=list(RootedTBRSwap, RootedSPRSwap, RootedNNISwap)))
...
write.nexus(iw.tree, file=paste0("hy_iw_k4_", round(IWScore(iw.tree, iw_data, concavity=k), 3), ".tre", collapse=''))
write.nexus(iw.consensus, file=paste0("hy_iw_k4_", round(IWScore(iw.tree, iw_data, concavity=k), 3), ".con.tre", collapse=''))

pdf(file=paste0("hy_iw_", round(IWScore(iw.tree, my_data, concavity=k), 3), ".pdf", collapse=''))
par(mar=rep(0.25, 4), cex=0.75) # make plot easier to read
plot(ape::consensus(iw.consensus))
dev.off()
