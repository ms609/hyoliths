...
RefExpand <- function (ref) {
  # Blacklisted titles
  if (!ref['Title'] %in% "Invertebrate zoology: a functional evolutionary approach") {
    ...
    ref['Secondary.Title'] <- 
      if (grepl("interpreting evolutionary relationships within early Rhynchonelliformea",
      ...
      } else if (grepl("palaeoecology of the trimerellid brachiopod",
                title, fixed=TRUE)) {
        "Palaeontology"
        ...
        ref['Secondary.Author'] <- "Lipps, J. H. and Signor, Philip W."
        ref['Publisher'] <- "Pergamon"
        "Origin and Early Evolution of Metazoa"
      } else ref['Secondary.Title']
    if (grepl('Fish Evolution and Systematics', title, fixed=TRUE)) {
      ref['Publisher'] <- "Cambridge University Press"
    }
    ...
    if (!is.null(ref['DOI']) && ref['DOI'] != "") {
      ...
      } else if ("Phoronida" == title) {
        ref['Pubilsher'] <- "Wiley-Blackwell"
