...
FirstWords <- function (text) gsub("(\\w+)\\W*(\\w+)?.*", "\\1\\2", trimws(text), perl=TRUE)
...
BibEntry <- function (ref) {
  ...
  paste0("@", FirstWord(ref['Reference.Type']), '{', 
         RefKey(ref), ",\n",
         BibLine('author', 'Author', gsub(";", " and ", ref["Author"], fixed=TRUE)),
         BibLine('doi', 'DOI'),
         BibLine('journal', 'Secondary.Title'),
         BibLine('title', 'Title', gsub("</?i>", "_", ref["Title"], perl=TRUE)),
