...
refs <- trimws(as.matrix(read.delim(mbRecent, sep="\t", header=TRUE, skip=1, row.names=NULL)))
...
cached_dois <- R.cache::addMemoization(function(doi)
  if (doi == '') NULL else rcrossref::cr_works(dois=doi, limit=1,                                               select='container-title,title,issue,volume,score'))
...
RefExpand <- function (ref) {
  ...
  RefAttribute <- function (name) {
    if (is.na(ref[name])) NULL else as.character(ref[name])
  }
  cr_details <- cached_dois(ref['DOI'])
  if (is.null(cr_details) || is.null(cr_details$meta) || cr_details$meta[, 'total_results'] == 0) {
      
    flq <- c(`query.container-title`= RefAttribute('Journal'), 
               query.author = RefAttribute('Author'),
               query.bibliographic = paste(
                  RefAttribute('Year'), 
                  RefAttribute('Volume'),
                  RefAttribute('issn'),
                  RefAttribute('Pages'), sep=" ;")
    )
    flq <- flq[!is.na(flq)]
    
    cr_details <- 
      cr_works(dois=,
                   query=CleanTitle(ref['Title']), flq=flq, limit=1,
                   select='container-title,DOI,title,issue,volume,score')
  }
  if (cr_details$meta[, 'total_results'] > 0) {
    topResult <- cr_details$data[1, ]
    if (as.numeric(topResult$score) > 50) {
